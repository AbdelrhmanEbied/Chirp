#!/usr/bin/env python3
"""Generate development and load-test data.

This writes directly into each service's database rather than going through
the public APIs. That is a deliberate exception to the "services own their
data" rule, made because the purpose of this script is to produce tens of
thousands of rows quickly: at HTTP speeds, 50 000 users would take hours and
would also rate-limit itself. It is a development tool, is never shipped in a
service image, and is the only code in the repository that opens more than one
service's database.

Usage:
    python scripts/seed.py --size small      #   200 users
    python scripts/seed.py --size medium     # 5 000 users
    python scripts/seed.py --size large      # 50 000 users
    python scripts/seed.py --users 1000 --posts-per-user 20

All generated accounts share one password so you can log in as any of them:
    email    ada0@chirp.local
    password chirp-development-password
"""
...
def database_url(service: str) -> str:
    """Build a per-service URL from the same variables Compose uses."""
...
async def seed_accounts_and_profiles(plan: Plan, batch_size: int = 1_000) -> list[str]:
    """Create auth accounts and matching user profiles, in bulk.

    The password is hashed once and reused. Argon2 is intentionally slow, so
    hashing 50 000 times would dominate the runtime and teach us nothing.
    """
...
async def main() -> int:
    ...
    # Posts, follows, likes and messages are generated by the same pattern once
    # those services land; see docs/local-development.md.
    return 0
