...
def settings() -> MediaSettings:
    return MediaSettings(
        service_name="media-service-test",
        environment="test",
        jwt_secret=TEST_JWT_SECRET,
        database_url=TEST_DATABASE_URL,
        event_bus_backend="memory",
        log_json=False,
        media_local_path="/tmp/chirp-test-media",
    )


@pytest_asyncio.fixture
async def context(settings: MediaSettings) -> AsyncIterator[ServiceContext]:
...
async def client(context: ServiceContext) -> AsyncIterator[AsyncClient]:
    ...
    app = create_app(settings=context.settings, title="media-test")
