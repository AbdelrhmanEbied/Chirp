...
class ServiceContext:
    ...
    @classmethod
    def create(cls, settings: ModerationSettings) -> "ServiceContext":
        return cls(
            settings=settings,
...
def get_moderation_service(context: Context, session: DbSession) -> ModerationService:
...
def _admin_user(request: Request) -> AuthenticatedUser:
    required, _ = build_auth_dependencies(get_context(request).codec)
    user = required(request)
    user.require_admin()
    return user


AdminUser = Annotated[AuthenticatedUser, Depends(_admin_user)]
