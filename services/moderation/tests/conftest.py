"""Test fixtures for the moderation service."""
...
@pytest.fixture
def settings() -> ModerationSettings:
    return ModerationSettings(
        service_name="moderation-service-test",
        environment="test",
...
async def context(settings: ModerationSettings) -> AsyncIterator[ServiceContext]:
...
async def client(context: ServiceContext) -> AsyncIterator[AsyncClient]:
    ...
    app = create_app(settings=context.settings, title="moderation-test")
    app.state.context = context
    app.include_router(router)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://moderation.test") as http:
        yield http
