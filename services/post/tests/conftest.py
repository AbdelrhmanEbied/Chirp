...
@pytest.fixture
def settings() -> PostSettings:
    return PostSettings(
        service_name="post-service-test",
        environment="test",
...
async def context(settings: PostSettings) -> AsyncIterator[ServiceContext]:
...
async def client(context: ServiceContext) -> AsyncIterator[AsyncClient]:
    ...
    app = create_app(settings=context.settings, title="post-test")
    app.state.context = context
    app.include_router(router)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://post.test") as http:
        yield http
...
@pytest.fixture
def user_client(context: ServiceContext):
    return None
