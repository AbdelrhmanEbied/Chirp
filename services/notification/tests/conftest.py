...
async def context(settings: NotificationSettings) -> AsyncIterator[ServiceContext]:
...
async def client(context: ServiceContext) -> AsyncIterator[AsyncClient]:
    ...
    app = create_app(settings=context.settings, title="notification-test")
    app.state.context = context
    app.include_router(router)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://notification.test") as http:
        yield http
