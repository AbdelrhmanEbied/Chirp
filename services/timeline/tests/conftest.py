"""Test fixtures for the timeline service."""
...
@pytest.fixture
def settings() -> TimelineSettings:
    return TimelineSettings(
        service_name="timeline-service-test",
        environment="test",
...
async def context(settings: TimelineSettings) -> AsyncIterator[ServiceContext]:
    ...
    ctx = ServiceContext(
        ...
        post_client=FakeServiceClient(),  # type: ignore[arg-type]
        graph_client=FakeServiceClient(),  # type: ignore[arg-type]
        user_client=FakeServiceClient(),  # type: ignore[arg-type]
        redis_client=None,
    )
    yield ctx
...
async def client(context: ServiceContext) -> AsyncIterator[AsyncClient]:
    ...
    app = create_app(settings=context.settings, title="timeline-test")
    app.state.context = context
    app.include_router(router)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://timeline.test") as http:
        yield http
